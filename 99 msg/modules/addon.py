...
def init_db(database, limit):
    sql = {
        "init_msg": "create table if not exists msg (id INTEGER PRIMARY KEY AUTOINCREMENT,name TEXT NOT NULL,msg TEXT NOT NULL,time TIMESTAMP DEFAULT CURRENT_TIMESTAMP,image blob,thumbnail blob)", "query": "select count(id) from msg",
        "limit": f"delete from msg where id in (select id from msg limit (select count(id) - {limit} from msg))"}
...
def db_crud(database, sql, prm, fetch, commit, query):
    with sqlite3.connect(database) as con:
        ...
        if query:
            ...
            # "query": "select count(id) from msg"
